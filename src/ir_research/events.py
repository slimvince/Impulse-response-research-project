...
def _onset_breaks(db: np.ndarray, frame_start: int, frame_stop: int, hop_size: int, sample_rate: int) -> list[int]:
    ...
    for index in range(1, len(rise) - 1):
        if rise[index] < 3.0 or rise[index] < rise[index - 1] or rise[index] < rise[index + 1]:
            continue
